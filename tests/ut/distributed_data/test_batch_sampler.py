...
from hyper_parallel.auto_models.components.datasets.batching.build_dataloader import (
...
class TestNativeBatchSampler(unittest.TestCase):
    ...
    def test_trainer_builder_reuses_sampler_and_collator(self) -> None:
        ...
        collator = object()
        with patch(
                "hyper_parallel.auto_models.components.datasets.batching.build_dataloader.build_distributed_dataloader",
        ) as build:
    ...
    def test_trainer_uses_rank_local_payload_device(self) -> None:
        ...
        with (
            patch(
                "hyper_parallel.auto_models.components.datasets.batching.build_dataloader.torch.cuda.current_device",
                return_value=3,
            ),
            patch(
                "hyper_parallel.auto_models.components.datasets.batching.build_dataloader.build_distributed_dataloader",
