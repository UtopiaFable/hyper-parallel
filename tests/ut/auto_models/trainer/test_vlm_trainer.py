...
from hyper_parallel.auto_models.components.datasets.batching import build_dataloader
from hyper_parallel.auto_models.components.data.vlm.collator import VLMCollator
...
class TestVLMTrainer(unittest.TestCase):
    ...
    def test_resume_crosses_epochs_and_drains_prefetch(self) -> None:
        ...
        with TemporaryDirectory() as directory:
            ...
            with patch.object(trainer, "train_step", side_effect=train_step), \
                    patch("hyper_parallel.auto_models.trainer.vlm_trainer.synchronize"), \
    ...
    def test_step_checkpoint_sees_completed_update(self) -> None:
        ...
        with patch("hyper_parallel.auto_models.trainer.vlm_trainer.synchronize"), \
                patch("hyper_parallel.auto_models.trainer.vlm_trainer.clip_grad_norm_", return_value=0.5):
